...
public class OutboxPublisherScheduler {
    ...
    public OutboxPublisherScheduler(
            ...
            @Value("${spring.kafka.topics.ticket-generated}") String topic,
    ...
    public void publishTicketGeneratedEvents() {
        List<OutboxEvent> events = outboxService.claimOutboxEvents(TICKET_GENERATED, EVENTS_LIMIT);
        for (OutboxEvent event : events) {
            try {
                ...
                log.error("Failed to publish ticket generated event {}", event.getId(), ex);
                outboxService.handleFailedAttempt(event, ex.getMessage());
            }
        }
    }
